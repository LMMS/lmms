...
// The core Base64 encoding and decoding methods are primarily used for storing arrays of floating-point samples, so the
// tests focus on those. A compatibility decoding method for an old data format is provided in lmms::base64, but it
// should be considered deprecated and is not tested here.
...
private slots:
	...
	void encodeDecodeStringTest()
	{
		...
		lmms::base64::encode(TEST_STRING, strlen(TEST_STRING) + 1,
			encodedString); // The + 1 is because the strlen function excludes the terminating character of the string.
	...
	void encodeDecodePositiveArrayTest()
	{
		...
		QVERIFY(memcmp(positiveFloats, decodedData, TEST_COUNT * sizeof(float))
			== 0); // Check that the original and decoded arrays are identical in memory. A value of 0 means that they
				   // are equal.
	...
	void encodeDecodeNegativeArrayTest()
	{
		...
		QCOMPARE(size, TEST_COUNT * (int)sizeof(float)); // Confirm that both arrays have the same length.
		QVERIFY(memcmp(negativeFloats, decodedData, TEST_COUNT * sizeof(float))
			== 0); // Check that the original and decoded arrays are identical in memory. A value of 0 means that they
				   // are equal.
