...
static gchar* get_property(Display *disp, Window win, Atom xa_prop_type, std::string prop_name, unsigned long *size) {
	Atom xa_prop_name;
	Atom xa_ret_type;
	int ret_format;
...
static unsigned long list_windows(Display *disp, unsigned long match_pid) {
...
class SubprocessWrapper : public QObject {
	...
	public:
		SubprocessWrapper(){};
		SubprocessWrapper(QString exe, QStringList args, bool capture=false, QWidget* parent=NULL, int width=320, int height=240) {
#ifdef __linux__
