...
class AudioFileFlacSample : public AudioFileDeviceSample
{
public:
	AudioFileFlacSample(OutputSettings const& outputSettings,
			bool& successful,
			const QString& file,
			const fpp_t defaultBufferSize);
	~AudioFileFlacSample() override;

	static AudioFileDeviceSample* getInst(
			OutputSettings const &outputSettings,
			bool &successful,
			const QString & outputFilename,
			const fpp_t defaultBufferSize)
	{
		return new AudioFileFlacSample(
			outputSettings,
			successful,
			outputFilename,
			defaultBufferSize
