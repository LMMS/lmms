...
public:
	...
	*/
	struct ChordSemiTone {
		 ...
		 * @param s silenced
		 */
		ChordSemiTone(int8_t k, volume_t v, panning_t p, bool a, bool s) {
			key = k;
			vol = v;
			pan = p;
			active = a;
			silenced = s;
			bare=false; //as we are defining the note the bare operand is automatically falsed
		}
		 ...
		 *
		 * 3, 100, 0, 1,1,0 the last parameter is the bare param: if true all others
		...
		inline int8_t operator=(int8_t a) {
			key = a;
			bare =
					true; // the note has only the key component, we discard all the rest
			return a;
		}
...
public:
	...
	public:
		...
		/**
		 * For the sake of compatibility returns the key
		 * @brief last
		 * @return
		 */
		...
		/**
		 * Gets the associated semitones
		 * @brief getChordSemiTones
		 * @return
		 */
		const ChordSemiTones getChordSemiTones() const { return m_semiTones; }

		/**
		 * returns the key, for compatibility issues!
		 */
	...
	private:
		...
		// This to populate the static QVector of data
...
//		static Init s_initTable[]; // Old initializer, relies on an array of data

		static Init s_initializer; // This is going to get the data!
