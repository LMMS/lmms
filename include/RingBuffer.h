...
public:
	...
	/** \brief Constructs a ringbuffer of specified samplerate-dependent size, which will be updated when samplerate
	 * changes \param size The size of the buffer in milliseconds. The actual size will be size + period size
	 */
	 ...
	 * 	\param length Length of the source buffer, if zero, period size is used - must not be higher than the size of
	 * the ringbuffer!
	 */
	 ...
	 * 	\param offset Offset in frames against current position, may *NOT* be negative
	 * 	\param length Length of the source buffer, if zero, period size is used - must not be higher than the size of
	 * the ringbuffer! \param level Multiplier applied to the frames before they're written to the ringbuffer
	 */
