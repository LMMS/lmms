...
class AudioFileDeviceSample
{
public:
	// getBufferFunction
	using BufferFn = std::function<void(SampleFrame*, fpp_t*, fpp_t)>;

	AudioFileDeviceSample(OutputSettings const & outputSettings,
			const QString & _file,
			const fpp_t defaultBufferSize);
	virtual ~AudioFileDeviceSample();
...
using AudioFileDeviceSampleInstantiaton
	= AudioFileDeviceSample* (*)(const OutputSettings&, bool&, const QString&, const fpp_t);
