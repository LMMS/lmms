...
public: \
	type getfunc() const { return (type)modelname->value(); } \
public slots: \
	void setfunc(const type val) { modelname->setValue(val); }

#define mapPropertyFromModel(type, getfunc, setfunc, modelname) \
public: \
	type getfunc() const { return (type)modelname.value(); } \
public slots: \
	void setfunc(const type val) { modelname.setValue((float)val); }
...
public:
	...
	*/
	...
	bool isAutomatedOrControlled() const { return isAutomated() || m_controllerConnection != NULL; }
	...
	void setScaleLogarithmic(bool setToTrue = true) { setScaleType(setToTrue ? Logarithmic : Linear); }
	bool isScaleLogarithmic() const { return m_scaleType == Logarithmic; }
...
protected:
	AutomatableModel(const float val = 0, const float min = 0, const float max = 0, const float step = 0,
		Model* parent = NULL, const QString& displayName = QString(), bool defaultConstructed = false);
