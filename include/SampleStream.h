...
public:
	...
	**/
	...
	SampleStream(const SampleStream& stream) = delete;
	SampleStream& operator=(const SampleStream& stream) = delete;
	...
	//! Starts the stream.
	//! Once called, a dedicated disk streaming thread will be started on the `ThreadPool`.
	void start();

	//! Stops the stream.
	//! Once called, the dedicated disk streaming thread stops running.
	void stop();
