...
public:
   /**
	   ...
	   @param title Title of the FileBrowser
	   @param pm QPixmap icon to be shown in the sidebar
	   @param parent Parent object
	   @param dirs_as_items //TODO *to be documented*
	   @param userDir Directory containing user content only. enables user content checbox
	   @param factoryDir Directory containing LMMS provided content only. enables factory content checbox
	   @param showDirs //TODO * to be documented*
	   @param items List of items (files or folders) to be shown.
   */
   ...
   [[nodiscard]] inline QString directories() {
	   return m_directories;
   }
   [[nodiscard]] inline QString userDir() {
	   return m_userDir;
   }
...
class FileBrowserTreeWidget : public QTreeWidget
{
   Q_OBJECT
public:
   FileBrowserTreeWidget( FileBrowser * fileBrowser);
...
private:
   ...
   static void handleFile( FileItem * f, InstrumentTrack * it );
   void openInNewInstrumentTrack( TrackContainer* tc, FileItem* item );


   FileBrowser* m_parentBrowser;
   bool m_mousePressed;
   QPoint m_pressPos;
...
public:
   ...
   Directory(QDir dir, const QString& filter, bool disableEntryPopulation = false);
