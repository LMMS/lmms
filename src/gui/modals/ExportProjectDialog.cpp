	...
	m_renderManager( nullptr )
{
	...
	int cbIndex = 0;
	for (auto i = std::size_t{0}; i < LmmsExporter::NumFileFormats; ++i)
	{
		if (LmmsExporter::s_fileEncodeDevices[i].isAvailable())
		{
			...
			fileFormatCB->addItem(ProjectRenderer::tr(
				LmmsExporter::s_fileEncodeDevices[i].m_description),
				QVariant(static_cast<int>(LmmsExporter::s_fileEncodeDevices[i].m_fileFormat)) // Format tag; later used for identification.
			);
...
void ExportProjectDialog::onFileFormatChanged(int index)
{
	...
	auto exportFormat = static_cast<LmmsExporter::ExportAudioFileFormat>(
		format_tag.toInt(&successful_conversion)
	);
	Q_ASSERT(successful_conversion);

	bool stereoModeVisible = (exportFormat == LmmsExporter::ExportAudioFileFormat::MP3);

	bool sampleRateControlsVisible = (exportFormat != LmmsExporter::ExportAudioFileFormat::MP3);

	bool bitRateControlsEnabled =
			(exportFormat == LmmsExporter::ExportAudioFileFormat::Ogg ||
			 exportFormat == LmmsExporter::ExportAudioFileFormat::MP3);

	bool bitDepthControlEnabled =
			(exportFormat == LmmsExporter::ExportAudioFileFormat::Wave ||
			 exportFormat == LmmsExporter::ExportAudioFileFormat::Flac);

	bool variableBitrateVisible = !(exportFormat == LmmsExporter::ExportAudioFileFormat::MP3 || exportFormat == LmmsExporter::ExportAudioFileFormat::Flac);

#ifdef LMMS_HAVE_SF_COMPLEVEL
	bool compressionLevelVisible = (exportFormat == LmmsExporter::ExportAudioFileFormat::Flac);
