...
#endif
{
	...
	if (m_ui)
	{
		...
		const QSizePolicy sizePolicyToSet = m_isResizable
			? QSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding)
			: QSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
		m_uiInstanceWidget->setSizePolicy(sizePolicyToSet);
		setSizePolicy(sizePolicyToSet);
		parent->setSizePolicy(sizePolicyToSet);  // TODO: remove when Lv2 UI removes Lv2ViewBase

		if(!m_isResizable)
			setFixedSize(m_uiInstanceWidget->size());
	...
	m_helpWindowEventFilter(this)
{
	...
	if(m_procView->minimumSize() == m_procView->maximumSize())
		meAsWidget->setFixedSize(m_procView->size());
}
