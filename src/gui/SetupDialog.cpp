...
SetupDialog::SetupDialog( ConfigTabs _tab_to_open ) :
	...
	m_defaultSF2File( QDir::toNativeSeparators( ConfigManager::inst()->defaultSF2File() ) ),
#endif
	...
	m_enableAutoSave( ConfigManager::inst()->value( "ui",
							"enableautosave", "1" ).toInt() ),
	m_enableRunningAutoSave( ConfigManager::inst()->value( "ui",
							"enablerunningautosave", "0" ).toInt() ),
	m_saveInterval(	ConfigManager::inst()->value( "ui",
							"saveinterval" ).toInt() < 1 ?
					MainWindow::DEFAULT_SAVE_INTERVAL_MINUTES :
					ConfigManager::inst()->value( "ui",
							"saveinterval" ).toInt() ),
	m_smoothScroll( ConfigManager::inst()->value( "ui",
							"smoothscroll" ).toInt() ),
						   ...
						   "disableautoquit").toInt() )
{
	...
	LedCheckBox * displayWaveform = new LedCheckBox(
				tr( "Enable master oscilloscope by default" ),
								gui_tw );
	labelNumber1++;
	...
	LedCheckBox * mmpz = new LedCheckBox(
				tr( "Compress project files by default" ),
								projects_tw );
	labelNumber2++;
	...
	LedCheckBox * disableBackup = new LedCheckBox(
				tr( "Create a backup file when saving a project" ),
								projects_tw );
	labelNumber2++;
	...
	LedCheckBox * openLastProject = new LedCheckBox(
				tr( "Reopen last project on startup" ),
								projects_tw );
	labelNumber2++;
	...
	labelWidget( performance_w, tr( "Performance settings" ) );
	...
	ToolTip::add( saveIntervalBtn, tr( "Help" ) );
	...
	LedCheckBox * animAFP = new LedCheckBox(
				tr( "Display playback cursor in AudioFileProcessor" ),
								ui_fx_tw );
	animAFP->move( 10, 40 );
	...
	labelWidget( audio_w, tr( "Audio settings" ) );
	...
	ToolTip::add( audio_help_btn, tr( "Help" ) );
	...
	// Buffer size tab.
	TabWidget * bufferSize_tw = new TabWidget( tr( "Buffer size" ),
									audio_w );
	...
	ToolTip::add( bufferSize_help_btn, tr( "Help" ) );
	...
	labelWidget( midi_w, tr( "MIDI settings" ) );
	...
	ToolTip::add( midi_help_btn, tr( "Help" ) );
...
//	QLabel * title = new QLabel( tr( "Paths" ), paths_w ); -> Commented for testing.
...
//	title->setFont( pointSize<12>( f ) ); -> Commented for testing.
	...
	TabWidget * workingDir_tw = new TabWidget( tr(
	...
	TabWidget * vstDir_tw = new TabWidget( tr(
	...
	TabWidget * ladspaDir_tw = new TabWidget( tr(
				"LADSPA plugins directories" ),
							paths_w );
	...
	TabWidget * sf2Dir_tw = new TabWidget( tr(
				"SF2 files directory" ),
								pathSelectors );
	...
	TabWidget * defaultSF2File_tw = new TabWidget( tr(
				"Default SF2 file" ), paths_w );
	defaultSF2File_tw->setFixedHeight( 48 );

	m_defaultSF2FileLineEdit = new QLineEdit( m_defaultSF2File, defaultSF2File_tw );
	m_defaultSF2FileLineEdit->setGeometry( 10, 20, txtLength, 16 );
	connect( m_defaultSF2FileLineEdit, SIGNAL( textChanged( const QString & ) ), this,
		 	SLOT( setDefaultSF2File( const QString & ) ) );

	QPushButton * defaultSF2File_select_btn = new QPushButton(
				embed::getIconPixmap( "project_open", 16, 16 ),
								"", defaultSF2File_tw );
	defaultSF2File_select_btn->setFixedSize( 24, 24 );
	defaultSF2File_select_btn->move( btnStart, 16 );
	connect( defaultSF2File_select_btn, SIGNAL( clicked() ), this,
			SLOT( openDefaultSF2File() ) );
#endif
		
	// GIG files directory tab.
	TabWidget * gigDir_tw = new TabWidget( tr(
				"GIG files directory" ),
								pathSelectors );
	...
	// Theme directory tab.
	TabWidget * themeDir_tw = new TabWidget( tr(
				"Theme direcroty" ),
								pathSelectors );
	...
	TabWidget * backgroundPicFile_tw = new TabWidget( tr(
				"Background picture file" ), paths_w );
	backgroundPicFile_tw->setFixedHeight( 48 );

	m_baLineEdit = new QLineEdit( m_backgroundPicFile, backgroundPicFile_tw );
	...
	// Major tabs ordering.
	m_tabBar->addTab( general_w, tr( "General settings" ), 0, false,
					true )->setIcon( embed::getIconPixmap( "setup_general" ) );
	m_tabBar->addTab( performance_w, tr( "Performance settings" ), 1, false,
					true )->setIcon( embed::getIconPixmap( "setup_performance" ) );
	m_tabBar->addTab( audio_w, tr( "Audio settings" ), 2, false,
					true )->setIcon( embed::getIconPixmap( "setup_audio" ) );
	m_tabBar->addTab( midi_w, tr( "MIDI settings" ), 3, false,
					true )->setIcon( embed::getIconPixmap( "setup_midi" ) );
	m_tabBar->addTab( paths_w, tr( "Paths" ), 4, true,
					true )->setIcon( embed::getIconPixmap( "setup_directories" ) );
	...
	LedCheckBox * restartWarning = new LedCheckBox(
				tr( "Display restart warning after changing settings" ),
				extras_w );
...
#ifdef LMMS_HAVE_FLUIDSYNTH
	ConfigManager::inst()->setDefaultSF2File( m_defaultSF2File );
#endif
...
{
	QString new_dir = FileDialog::getExistingDirectory( this,
				tr( "Choose your VST plugins directory" ),
							m_vstDir );
	if( new_dir != QString::null )
	{
...
{
	QString new_dir = FileDialog::getExistingDirectory( this,
				tr( "Choose your LADSPA plugins directory" ),
							m_ladspaDir );
	if( new_dir != QString::null )
	{
...
{
	QString new_dir = FileDialog::getExistingDirectory( this,
				tr( "Choose your SF2 files directory" ),
							m_sf2Dir );
	if( new_dir != QString::null )
	{
...
void SetupDialog::openDefaultSF2File()
{
#ifdef LMMS_HAVE_FLUIDSYNTH
	QString new_file = FileDialog::getOpenFileName( this,
				tr( "Choose your default SF2 file" ), m_defaultSF2File, 
				"SoundFont 2 files (*.sf2)" );
	
	if( new_file != QString::null )
	{
		m_defaultSF2FileLineEdit->setText( new_file );
	}
#endif
}


void SetupDialog::setDefaultSF2File( const QString & defaultSF2File )
{
#ifdef LMMS_HAVE_FLUIDSYNTH
	m_defaultSF2File = defaultSF2File;
#endif
}
...
{
	QString new_dir = FileDialog::getExistingDirectory( this,
				tr( "Choose your GIG files directory" ),
							m_gigDir );
	if( new_dir != QString::null )
	{
...
{
	QString new_dir = FileDialog::getExistingDirectory( this,
				tr( "Choose your theme directory" ),
							m_themeDir );
	if( new_dir != QString::null )
	{
...
void SetupDialog::openBackgroundPicFile()
{
	...
	QString dir = ( m_backgroundPicFile.isEmpty() ) ?
		...
		m_backgroundPicFile;
	QString new_file = FileDialog::getOpenFilename( this,
			tr( "Choose your background picture file" ), dir, 
