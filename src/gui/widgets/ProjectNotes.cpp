	...
	: QMainWindow(gui->mainWindow()->workspace())
{
	...
	connect(m_edit, SIGNAL(currentCharFormatChanged(const QTextCharFormat&)), this,
		SLOT(formatChanged(const QTextCharFormat&)));
	//	connect( m_edit, SIGNAL( currentAlignmentChanged( int ) ),
	//			this, SLOT( alignmentChanged( int ) ) );
	connect(m_edit, SIGNAL(textChanged()), Engine::getSong(), SLOT(setModified()));
...
void ProjectNotes::setupActions()
{
	...
	connect(m_comboFont, SIGNAL(activated(const QString&)), m_edit, SLOT(setFontFamily(const QString&)));
	...
	m_actionTextBold = new QAction(embed::getIconPixmap("text_bold"), tr("&Bold"), this);
	...
	m_actionTextItalic = new QAction(embed::getIconPixmap("text_italic"), tr("&Italic"), this);
	...
	m_actionTextUnderline = new QAction(embed::getIconPixmap("text_under"), tr("&Underline"), this);
	...
	m_actionAlignLeft = new QAction(embed::getIconPixmap("text_left"), tr("&Left"), m_edit);
	m_actionAlignLeft->setShortcut(tr("%1+L").arg(UI_CTRL_KEY));
	m_actionAlignLeft->setCheckable(true);
	grp->addAction(m_actionAlignLeft);

	m_actionAlignCenter = new QAction(embed::getIconPixmap("text_center"), tr("C&enter"), m_edit);
	...
	m_actionAlignRight = new QAction(embed::getIconPixmap("text_right"), tr("&Right"), m_edit);
	...
	m_actionAlignJustify = new QAction(embed::getIconPixmap("text_block"), tr("&Justify"), m_edit);
