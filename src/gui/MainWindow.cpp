   ...
   m_session( SessionState::Normal )
{
   ...
   m_innerWidget = new QWidget(main_widget);
   auto hbox = new QHBoxLayout(m_innerWidget);
   hbox->setSpacing( 0 );
   hbox->setContentsMargins(0, 0, 0, 0);

   m_sideBar = new SideBar(Qt::Vertical, m_innerWidget);

   auto splitter = new QSplitter(Qt::Horizontal, m_innerWidget);
   ...
   m_sideBar->appendTab( new PluginBrowser( splitter ) );
   emit initProgress(tr("Preparing file browsers"));
   m_sideBar->appendTab( new FileBrowser(
   ...
   m_sideBar->appendTab( new FileBrowser(
	   ...
	   confMgr->userPresetsDir(),
	   confMgr->factoryPresetsDir()));
   m_sideBar->appendTab(new FileBrowser(QDir::homePath(), FileItem::defaultFilters(), tr("My Home"),
   ...
   m_sideBar->appendTab(new FileBrowser(root_paths.join("*"), FileItem::defaultFilters(), title,
	   embed::getIconPixmap("computer").transformed(QTransform().rotate(90)), splitter, m_dirs_as_items));

   m_starredItemBrowser = new FileBrowser(
	   "",
	   FileItem::defaultFilters(),
	   "Starred Items",
	   ...
	   "",
	   "",
	   confMgr->starredItems()
		   );

   m_sideBar->appendTab(
	  m_starredItemBrowser
   );
   ...
   if (confMgr->value("tooltips", "disabled").toInt())
   {
