   ...
   m_items(items)
{
   ...
   m_fileBrowserTreeWidget = new FileBrowserTreeWidget( this );
   addContentWidget( m_fileBrowserTreeWidget );

   m_searchTreeWidget = new FileBrowserTreeWidget(this);
...
void FileBrowser::reloadTree()
{
   ...
   // if the string is empty make an empty list. this way it won't have an Empty element.
...
void FileBrowser::addEntry(const QFileInfo & entry, const QString & path) {
	if (FileBrowser::excludedPaths().contains(entry.absoluteFilePath())) { return; }

	// Normalize the path
	QString normalizedPath = QDir::cleanPath(entry.absoluteFilePath());
...
FileBrowserTreeWidget::FileBrowserTreeWidget(FileBrowser * fileBrowser) :
   QTreeWidget( fileBrowser->contentParent() ),
   m_parentBrowser(fileBrowser),
...
void FileBrowserTreeWidget::keyPressEvent(QKeyEvent * ke )
{
   ...
   // If it's not a file, try to expand it.
   if (file == nullptr) {
	   if (preview && dynamic_cast<Directory*>(item) != nullptr) {
...
void FileBrowserTreeWidget::contextMenuEvent(QContextMenuEvent * e )
{
...
#else
	QString fileManager = tr("file manager"); // note: this is intentionally not capitalized as it isn't a proper noun
#endif
   ...
   if( file != nullptr)
   {
   	...
   	std::function<void()> starAction = !starred
			? std::function<void()>([=]{ mainWindow->starItem(file->fullName()); })
			: std::function<void()>([=]{ mainWindow->unstarItem(file->fullName()); });


	   // a star or pin icon would be nice
	   contextMenu.addAction(
		   !starred ? tr("Star") : tr("Unstar") + " " + tr("file"),
		   starAction
	   );
	   ...
	   contextMenu.addAction(
		 ...
		 ? std::function<void()>([=]{ mainWindow->starItem(dir->fullName()); })
		 : std::function<void()>([=]{ mainWindow->unstarItem(dir->fullName()); });

	   // a star or pin icon would be nice
	   contextMenu.addAction(
		   !starred ? tr("Star") : tr("Unstar") + " " + tr("folder"),
		   dirStarAction
	   );
   }
...
Directory::Directory(QDir dir, const QString& filter, bool disableEntryPopulation)
   : QTreeWidgetItem(QStringList(dir.dirName()), TypeDirectoryItem)
