...
void InstrumentTrack::processCCEvent(int controller)
{
	...
	// TODO: Decide whether we want MIDI CC rack control events to be interpreted as external (coming
	// from a hardware controller) or internal (generated by the instrument track). If they are to be
	// interpreted as external, processInEvent should be used instead. In this case the instrument track
	// will behave EXACTLY as if a hardware MIDI controller sent the events to it, processing some of the
	// events and forwarding the rest to the instrument plugin with handleMidiEvent. The events in this case
	// are not further forwarded with MidiPorts. If we decide to interpret CC events as internal they won't
	// be interpreted as CC events coming from hardware (the instrument track is not going to do any preliminar 
	// processing like for the Sustain CC message for example), but instead ALL CC events will be generated
	// and further forwarded by processOutEvent to the instrument plugin and any other track that is connected
	// through the MIDI ports to this one. Those are the ones that will do what they will with the event.
