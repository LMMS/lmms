...
#if ! defined (hiir_HalfBand8Avx_CODEHEADER_INCLUDED)
#define hiir_HalfBand8Avx_CODEHEADER_INCLUDED
...
template <int NC>
constexpr int 	HalfBand8Avx <NC>::_nbr_chn;
template <int NC>
constexpr int 	HalfBand8Avx <NC>::NBR_COEFS;
template <int NC>
constexpr double	HalfBand8Avx <NC>::_delay;
...
template <int NC>
HalfBand8Avx <NC>::HalfBand8Avx () noexcept
:	_bifilter ()
,	_phase (0)
{
...
template <int NC>
void	HalfBand8Avx <NC>::set_coefs (const double coef_arr []) noexcept
{
	assert (coef_arr != nullptr);

	for (int i = 0; i < NBR_COEFS; ++i)
	{
		const auto     c = _mm256_set1_ps (DataType (coef_arr [i]));
...
template <int NC>
__m256	HalfBand8Avx <NC>::process_sample (__m256 input) noexcept
{
	const auto     path_arr = process_2_paths (input);
	const auto     low      = _mm256_add_ps (path_arr._e, path_arr._o);
...
template <int NC>
void	HalfBand8Avx <NC>::process_block (float out_ptr [], const float in_ptr [], long nbr_spl) noexcept
{
...
template <int NC>
__m256	HalfBand8Avx <NC>::process_sample_hpf (__m256 input) noexcept
{
	const auto     path_arr = process_2_paths (input);
	const auto     high     = _mm256_sub_ps (path_arr._e, path_arr._o);
...
template <int NC>
void	HalfBand8Avx <NC>::process_block_hpf (float out_ptr [], const float in_ptr [], long nbr_spl) noexcept
{
	assert (out_ptr != nullptr);
	assert (out_ptr <= in_ptr || out_ptr >= in_ptr + nbr_spl * _nbr_chn);
...
template <int NC>
void	HalfBand8Avx <NC>::process_sample_split (__m256 &low, __m256 &high, __m256 input) noexcept
{
	const auto     path_arr = process_2_paths (input);
	low  = _mm256_add_ps (path_arr._e, path_arr._o);
	high = _mm256_sub_ps (path_arr._e, path_arr._o);
}
...
template <int NC>
void	HalfBand8Avx <NC>::process_block_split (float out_l_ptr [], float out_h_ptr [], const float in_ptr [], long nbr_spl) noexcept
{
...
template <int NC>
void	HalfBand8Avx <NC>::clear_buffers () noexcept
{
	_phase = 0;
	for (int i = 0; i < NBR_COEFS + 2; ++i)
	{
...
template <int NC>
constexpr int	HalfBand8Avx <NC>::_nbr_phases;
...
template <int NC>
typename HalfBand8Avx <NC>::EvenOdd	HalfBand8Avx <NC>::process_2_paths (__m256 input) noexcept
{
	const auto     half = _mm256_set1_ps (0.5f);
	input = _mm256_mul_ps (input, half);

	auto           path_arr = EvenOdd { input, _mm256_load_ps (_prev) };
...
void	HalfBand8Avx <NC>::process_block_2_paths (float out_l_ptr [], float out_h_ptr [], const float in_ptr [], long nbr_spl, FL fnc_l, FH fnc_h) noexcept
{
	...
	if (_phase == 1)
	{
		const auto     path_arr = process_2_paths (_mm256_loadu_ps (in_ptr));
	...
	auto           prev = _mm256_load_ps (_prev);
	while (pos < end)
	{
		const auto     ofs_0   = pos * _nbr_chn;
		const auto     ofs_1   = ofs_0 + _nbr_chn;
		auto           input_0 = _mm256_loadu_ps (in_ptr + ofs_0);
		auto           input_1 = _mm256_loadu_ps (in_ptr + ofs_1);

		input_0 = _mm256_mul_ps (input_0, half);
		input_1 = _mm256_mul_ps (input_1, half);
	...
	if (pos < nbr_spl)
	{
		...
		const auto     path_arr = process_2_paths (_mm256_loadu_ps (in_ptr + ofs));
...
template <int NC>
void	HalfBand8Avx <NC>::store_low (float *ptr, __m256 tmp_0, __m256 tmp_1) noexcept
{
	const auto     low = _mm256_add_ps (tmp_0, tmp_1);
	_mm256_storeu_ps (ptr, low);
}



template <int NC>
void	HalfBand8Avx <NC>::store_high (float *ptr, __m256 tmp_0, __m256 tmp_1) noexcept
{
	const auto     high = _mm256_sub_ps (tmp_0, tmp_1);
	_mm256_storeu_ps (ptr, high);
}
...
#endif   // hiir_HalfBand8Avx_CODEHEADER_INCLUDED
