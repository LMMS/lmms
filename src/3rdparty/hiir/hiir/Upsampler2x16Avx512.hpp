...
#if ! defined (hiir_Upsampler2x16Avx512_CODEHEADER_INCLUDED)
#define hiir_Upsampler2x16Avx512_CODEHEADER_INCLUDED
...
:	_filter ()
{
	...
	{
		_mm512_store_ps (_filter [i]._coef, _mm512_setzero_ps ());
	}
...
void	Upsampler2x16Avx512 <NC>::set_coefs (const double coef_arr [NBR_COEFS]) noexcept
{
	...
	{
		_mm512_store_ps (
			_filter [i + 2]._coef, _mm512_set1_ps (DataType (coef_arr [i]))
		);
	}
}
...
void	Upsampler2x16Avx512 <NC>::process_block (float out_ptr [], const float in_ptr [], long nbr_spl) noexcept
{
	...
	do
	{
		__m512         dst_0;
		__m512         dst_1;
		const __m512   src = _mm512_loadu_ps (in_ptr + pos * _nbr_chn);
		process_sample (dst_0, dst_1, src);
		_mm512_storeu_ps (out_ptr + pos * (_nbr_chn * 2)           , dst_0);
		_mm512_storeu_ps (out_ptr + pos * (_nbr_chn * 2) + _nbr_chn, dst_1);
		++ pos;
	}
	while (pos < nbr_spl);
}
...
void	Upsampler2x16Avx512 <NC>::clear_buffers () noexcept
{
	for (int i = 0; i < NBR_COEFS + 2; ++i)
	{
		_mm512_store_ps (_filter [i]._mem, _mm512_setzero_ps ());
	}
}
...
#endif   // hiir_Upsampler2x16Avx512_CODEHEADER_INCLUDED
