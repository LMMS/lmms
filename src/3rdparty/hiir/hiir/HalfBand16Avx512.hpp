...
#if ! defined (hiir_HalfBand16Avx512_CODEHEADER_INCLUDED)
#define hiir_HalfBand16Avx512_CODEHEADER_INCLUDED
...
template <int NC>
constexpr int 	HalfBand16Avx512 <NC>::_nbr_chn;
template <int NC>
constexpr int 	HalfBand16Avx512 <NC>::NBR_COEFS;
template <int NC>
constexpr double	HalfBand16Avx512 <NC>::_delay;
...
template <int NC>
HalfBand16Avx512 <NC>::HalfBand16Avx512 () noexcept
:	_bifilter ()
,	_phase (0)
{
...
template <int NC>
void	HalfBand16Avx512 <NC>::set_coefs (const double coef_arr []) noexcept
{
	assert (coef_arr != nullptr);

	for (int i = 0; i < NBR_COEFS; ++i)
	{
		const auto     c = _mm512_set1_ps (DataType (coef_arr [i]));
...
template <int NC>
__m512	HalfBand16Avx512 <NC>::process_sample (__m512 input) noexcept
{
	const auto     path_arr = process_2_paths (input);
	const auto     low      = _mm512_add_ps (path_arr._e, path_arr._o);
...
template <int NC>
void	HalfBand16Avx512 <NC>::process_block (float out_ptr [], const float in_ptr [], long nbr_spl) noexcept
{
...
template <int NC>
__m512	HalfBand16Avx512 <NC>::process_sample_hpf (__m512 input) noexcept
{
	const auto     path_arr = process_2_paths (input);
	const auto     high = _mm512_sub_ps (path_arr._e, path_arr._o);
...
template <int NC>
void	HalfBand16Avx512 <NC>::process_block_hpf (float out_ptr [], const float in_ptr [], long nbr_spl) noexcept
{
	assert (out_ptr != nullptr);
	assert (out_ptr <= in_ptr || out_ptr >= in_ptr + nbr_spl * _nbr_chn);
...
template <int NC>
void	HalfBand16Avx512 <NC>::process_sample_split (__m512 &low, __m512 &high, __m512 input) noexcept
{
	const auto     path_arr = process_2_paths (input);
	low  = _mm512_add_ps (path_arr._e, path_arr._o);
	high = _mm512_sub_ps (path_arr._e, path_arr._o);
}
...
template <int NC>
void	HalfBand16Avx512 <NC>::process_block_split (float out_l_ptr [], float out_h_ptr [], const float in_ptr [], long nbr_spl) noexcept
{
...
template <int NC>
void	HalfBand16Avx512 <NC>::clear_buffers () noexcept
{
	_phase = 0;
	for (int i = 0; i < NBR_COEFS + 2; ++i)
	{
...
template <int NC>
constexpr int	HalfBand16Avx512 <NC>::_nbr_phases;
...
template <int NC>
typename HalfBand16Avx512 <NC>::EvenOdd	HalfBand16Avx512 <NC>::process_2_paths (__m512 input) noexcept
{
	const auto     half = _mm512_set1_ps (0.5f);
	input = _mm512_mul_ps (input, half);

	auto           path_arr = EvenOdd { input, _mm512_load_ps (_prev) };

	StageProc16Avx512 <NBR_COEFS>::process_sample_pos (
...
void	HalfBand16Avx512 <NC>::process_block_2_paths (float out_l_ptr [], float out_h_ptr [], const float in_ptr [], long nbr_spl, FL fnc_l, FH fnc_h) noexcept
{
	...
	if (_phase == 1)
	{
		const auto     path_arr = process_2_paths (_mm512_loadu_ps (in_ptr));
	...
	auto           prev = _mm512_load_ps (_prev);
	while (pos < end)
	{
		const auto     ofs_0   = pos * _nbr_chn;
		const auto     ofs_1   = ofs_0 + _nbr_chn;
		auto           input_0 = _mm512_loadu_ps (in_ptr + ofs_0);
		auto           input_1 = _mm512_loadu_ps (in_ptr + ofs_1);

		input_0 = _mm512_mul_ps (input_0, half);
		input_1 = _mm512_mul_ps (input_1, half);
	...
	if (pos < nbr_spl)
	{
		...
		const auto     path_arr = process_2_paths (_mm512_loadu_ps (in_ptr + ofs));
...
template <int NC>
void	HalfBand16Avx512 <NC>::store_low (float *ptr, __m512 tmp_0, __m512 tmp_1) noexcept
{
	const auto     low = _mm512_add_ps (tmp_0, tmp_1);
	_mm512_storeu_ps (ptr, low);
}



template <int NC>
void	HalfBand16Avx512 <NC>::store_high (float *ptr, __m512 tmp_0, __m512 tmp_1) noexcept
{
	const auto     high = _mm512_sub_ps (tmp_0, tmp_1);
	_mm512_storeu_ps (ptr, high);
}
...
#endif   // hiir_HalfBand16Avx512_CODEHEADER_INCLUDED
