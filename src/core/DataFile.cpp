...
const std::vector<ProjectVersion> DataFile::UPGRADE_VERSIONS = {"0.2.1-20070501", "0.2.1-20070508", "0.3.0-rc2",
	"0.3.0", "0.4.0-20080104", "0.4.0-20080118", "0.4.0-20080129", "0.4.0-20080409", "0.4.0-20080607", "0.4.0-20080622",
	"0.4.0-beta1", "0.4.0-rc2", "1.0.99-0", "1.1.0-0", "1.1.91-0", "1.2.0-rc3", "1.3.0"};
...
const auto s_types = std::array{TypeDescStruct{DataFile::Type::Unknown, "unknown"},
	...
	, m_fileVersion(UPGRADE_METHODS.size())
{
	...
	if (!inFile.open(QIODevice::ReadOnly))
	{
		...
		{
			QMessageBox::critical(nullptr, gui::SongEditor::tr("Could not open file"),
...
QString DataFile::nameWithExtension(const QString& _fn) const
{
	...
	case Type::SongProject:
		...
		break;
	case Type::SongProjectTemplate:
		if (extension != "mpt") { return _fn + ".mpt"; }
		break;
	case Type::InstrumentTrackSettings:
		if (extension != "xpf") { return _fn + ".xpf"; }
		break;
	default:;
	}
	return _fn;
...
{
	if (type() == Type::SongProject || type() == Type::SongProjectTemplate || type() == Type::InstrumentTrackSettings)
	{
...
DataFile::Type DataFile::type(const QString& typeName)
{
	const auto it = std::find_if(
		s_types.begin(), s_types.end(), [&typeName](const TypeDescStruct& type) { return type.m_name == typeName; });
	if (it != s_types.end()) { return it->m_type; }

	// compat code
	if (typeName == "channelsettings") { return Type::InstrumentTrackSettings; }
...
void DataFile::upgrade_0_2_1_20070501()
{
	...
	for (int i = 0; !list.item(i).isNull(); ++i)
	{
		...
		if (el.hasAttribute("arpdir"))
		{
			...
			else { el.setAttribute("arpdisabled", "1"); }
		}
	}
...
void DataFile::upgrade_1_1_91()
{
	...
	for (int i = 0; !list.item(i).isNull(); ++i)
	{
		...
		else if (el.attribute("arpdir") == "4") { el.setAttribute("arpdir", "3"); }
	}
}
		...
		if (effect.attribute("name") == "ladspaeffect")
		{
			...
			for (int j = 0; !keys.item(j).isNull(); ++j)
			{
				...
				for (int k = 0; !attributes.item(k).isNull(); ++k)
				{
					...
					if (newName != pluginNames.end()) { attribute.setAttribute("value", newName->second); }
					...
					else if (plugin == "Equalizer5Band" || plugin == "Equalizer8Band" || plugin == "Equalizer12Band")
					{
						...
						auto fn = [&](QDomElement& port, int num, QList<QDomElement>& addList, QList<QDomElement>&) {
							if (num == 4)
							{
								...
								int zoom_port;
								if (plugin == "Equalizer5Band") zoom_port = 36;
					...
					else if (plugin == "Saturator")
					{
						auto fn = [&](QDomElement& port, int num, QList<QDomElement>&, QList<QDomElement>&) {
							...
							else if (num == 12) { port.setTagName("port016"); }
							else if (num == 13) { port.setTagName("port017"); }
							else if (num >= 15)
							{
								QString name("port0");
								name.append(QString::number(num + 3));
...
void DataFile::upgrade_bbTcoRename()
{
	...
	for (int i = 0; !elements.item(i).isNull(); ++i)
	{
		...
		static_assert(
			Track::Type::Pattern == static_cast<Track::Type>(1), "Must be type=1 for backwards compatibility");
...
void DataFile::upgrade_sampleAndHold()
{
	...
	for (int i = 0; i < elements.length(); ++i)
	{
		...
		if (e.attribute("wave").toInt() == 6) { e.setAttribute("speed", 0.01f); }
	}
}
...
void DataFile::loadData(const QByteArray& _data, const QString& _sourceFile)
{
	...
	if (root.hasAttribute("creatorversion"))
	{
			...
			&& gui::getGUI() != nullptr && root.attribute("type") == "song")
		{
			...
			gui::TextFloat::displayMessage(SongEditor::tr("Version difference"),
				SongEditor::tr("This %1 was created with LMMS %2").arg(projectType).arg(createdWith.getVersion()),
				embed::getIconPixmap("whatsthis", 24, 24), 2500);
		}
	}
