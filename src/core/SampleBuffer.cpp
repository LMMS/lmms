...
{
	// We already called update() in SampleBuffer()
}
...
SampleBuffer::SampleBuffer( const f_cnt_t _frames )
	: SampleBuffer(
			DataVector(static_cast<typename DataVector::size_type>(_frames),
			sampleFrame{0,0})
		  )
{
}
...
void SampleBuffer::update(bool _keep_settings)
{
	...
	else if( !m_audioFile.isEmpty() )
	{
		...
		if( !fileLoadError )
		{
			...
			{
				dataFrames = decodeSampleOGGVorbis( f, buf, channels, samplerate );
			}
#endif
			if( dataFrames == 0 )
			{
				dataFrames = decodeSampleSF( f, fbuf, channels,
									samplerate );
			}
#ifdef LMMS_HAVE_OGGVORBIS
			if( dataFrames == 0 )
			{
				dataFrames = decodeSampleOGGVorbis( f, buf, channels,
									samplerate );
			}
#endif
			if( dataFrames == 0 )
			{
				dataFrames = decodeSampleDS( f, buf, channels,
									samplerate );
			}
