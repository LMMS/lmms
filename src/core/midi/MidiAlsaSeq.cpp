...
MidiAlsaSeq::MidiAlsaSeq()
	...
	snd_seq_queue_tempo_set_tempo(tempo, 6000000 / Engine::getSong()->getTempo());
	...
	connect(Engine::getSong(), SIGNAL(tempoChanged(bpm_t)), this, SLOT(changeQueueTempo(bpm_t)), Qt::DirectConnection);

	// initial list-update
	updatePortList();

	connect(&m_portListUpdateTimer, SIGNAL(timeout()), this, SLOT(updatePortList()));
	// we check for port-changes every second
	m_portListUpdateTimer.start(1000);

	// use a pipe to detect shutdown
	if (pipe(m_pipe) == -1) { perror("MidiAlsaSeq: pipe"); }
...
void MidiAlsaSeq::processOutEvent(const MidiEvent& event, const TimePos& time, const MidiPort* port) {
	...
	snd_seq_event_t ev;
	snd_seq_ev_clear(&ev);
	snd_seq_ev_set_source(&ev, (m_portIDs[p][1] != -1) ? m_portIDs[p][1] : m_portIDs[p][0]);
	...
	case MidiKeyPressure:
		snd_seq_ev_set_keypress(&ev, event.channel(), event.key() + KeysPerOctave, event.velocity());
		break;
...
void MidiAlsaSeq::run() {
	...
	snd_seq_poll_descriptors(m_seqHandle, pollfd_set + 1, pollfd_count, POLLIN);
	...
	while (m_quit == false) {
		...
		while (snd_seq_event_input_pending(m_seqHandle, true) > 0) {
			...
			case SND_SEQ_EVENT_NOTEON:
				dest->processInEvent(MidiEvent(MidiNoteOn, ev->data.note.channel, ev->data.note.note - KeysPerOctave,
										 ev->data.note.velocity, source),
					TimePos(ev->time.tick));
				break;

			case SND_SEQ_EVENT_NOTEOFF:
				dest->processInEvent(MidiEvent(MidiNoteOff, ev->data.note.channel, ev->data.note.note - KeysPerOctave,
										 ev->data.note.velocity, source),
					TimePos(ev->time.tick));
				break;

			case SND_SEQ_EVENT_KEYPRESS:
				dest->processInEvent(MidiEvent(MidiKeyPressure, ev->data.note.channel,
										 ev->data.note.note - KeysPerOctave, ev->data.note.velocity, source),
					TimePos());
				break;

			case SND_SEQ_EVENT_CONTROLLER:
				dest->processInEvent(MidiEvent(MidiControlChange, ev->data.control.channel, ev->data.control.param,
			...
			case SND_SEQ_EVENT_PGMCHANGE:
				dest->processInEvent(
					MidiEvent(MidiProgramChange, ev->data.control.channel, ev->data.control.value, 0, source),
					TimePos());
				break;

			case SND_SEQ_EVENT_CHANPRESS:
				dest->processInEvent(MidiEvent(MidiChannelPressure, ev->data.control.channel, ev->data.control.param,
										 ev->data.control.value, source),
					TimePos());
				break;
			...
			case SND_SEQ_EVENT_PITCHBEND:
				dest->processInEvent(
					MidiEvent(MidiPitchBend, ev->data.control.channel, ev->data.control.value + 8192, 0, source),
					TimePos());
				break;
