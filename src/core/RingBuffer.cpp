...
void RingBuffer::setSamplerateAware( bool b )
{
	if( b )
	{
		connect( LmmsEngine::mixer(), SIGNAL( sampleRateChanged() ), this, SLOT( updateSamplerate() ), Qt::UniqueConnection );
	}
	else
	{
		disconnect( LmmsEngine::mixer(), SIGNAL( sampleRateChanged() ), this, SLOT( updateSamplerate() ) );
	}
}
...
{
	float newsize = static_cast<float>( ( m_size - m_fpp ) * LmmsEngine::mixer()->processingSampleRate() ) / m_samplerate;
