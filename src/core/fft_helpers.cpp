...
int precomputeWindow(float *window, unsigned int length, FFTWindow type, bool normalized)
{
	...
	{
		window[i] =	(a0 - a1 * cos(2 * numbers::pi_v<float> * i / ((float)length - 1.0))
			+ a2 * cos(4 * numbers::pi_v<float> * i / ((float)length - 1.0))
			- a3 * cos(6 * numbers::pi_v<float> * i / ((float)length - 1.0)));
		gain += window[i];
	}
