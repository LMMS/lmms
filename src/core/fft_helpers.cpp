...
int calc13octaveband31(float *absspec_buffer, float *subbands, int num_spec, float max_frequency)
{
	...
	for (int i = 0; i < 31; i++)
	{
		...
		float frequency = onethirdoctavecenterfr[i];

		float bandwidth = (pow(2, 1.0/3.0)-1) * frequency;

		float f_min = frequency - bandwidth / 2.0;
		float f_max = frequency + bandwidth / 2.0;

		int j_min = (int)(f_min / max_frequency * (float)num_spec);
		int j_max = (int)(f_max / max_frequency * (float)num_spec);
