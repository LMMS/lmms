...
static void context_state_callback(pa_context *c, void *userdata)
{
	...
	switch( pa_context_get_state( c ) )
	{
		...
		{
			LOG_INFO_TOPIC(PulseAudio, "Connection established");
...
void AudioPulseAudio::run()
{
	...
	if( !mainLoop )
	{
		LOG_ERR_TOPIC(PulseAudio, "pa_mainloop_new() failed" );
		return;
	}
	...
	if ( context == nullptr )
	{
		LOG_ERR_TOPIC(PulseAudio, "pa_context_new() failed" );
		return;
	}
