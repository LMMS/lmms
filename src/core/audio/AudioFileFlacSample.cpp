...
AudioFileFlacSample::AudioFileFlacSample(OutputSettings const& outputSettings,
			bool& successful,
			const QString& file,
			const fpp_t defaultBufferSize) :
	AudioFileDeviceSample(outputSettings, file, defaultBufferSize),
	m_sf(nullptr)
{
	successful = outputFileOpened() && startEncoding();
}

AudioFileFlacSample::~AudioFileFlacSample()
{
	finishEncoding();
}

bool AudioFileFlacSample::startEncoding()
{
...
void AudioFileFlacSample::finishEncoding()
{
	if (m_sf)
	{
		sf_write_sync(m_sf);
