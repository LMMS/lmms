...
void MixerChannel::doProcessing()
{
	...
	if( m_muted == false )
	{
		...
		m_peakLeft = std::max(m_peakLeft.load(std::memory_order_relaxed), peakSamples.left * v);
		m_peakRight = std::max(m_peakRight.load(std::memory_order_relaxed), peakSamples.right * v);
	}
	else
	{
