	...
	m_tempoSyncKnobModel(0, 0, 0, 1, m_port->max, this, m_port->name)
{
	...
	case FLOATING:
		m_knobModel.setRange(m_port->min, m_port->max,
			(m_port->max - m_port->min) / (m_port->name.toUpper() == "GAIN" && m_port->max == 10.0f ? 4000.0f : (m_port->suggests_logscale ? 8000000.0f : 800000.0f)));
...
{
	emit changed(m_port->port_id, static_cast<LADSPA_Data>(m_toggledModel.value()));
}

void LadspaControl::knobChanged()
{
	emit changed(m_port->port_id, static_cast<LADSPA_Data>(m_knobModel.value()));
}
