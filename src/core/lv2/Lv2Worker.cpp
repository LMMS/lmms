...
LV2_Worker_Status Lv2Worker::respond(uint32_t size, const void* data)
{
	if(m_threaded)
	{
		...
		{
			m_responses.enqueue(reinterpret_cast<const char*>(&size), sizeof(size));
			if(size && data) { m_responses.enqueue(reinterpret_cast<const char*>(data), size); }
		}
	}
	else
	{
...
void Lv2Worker::workerFunc()
{
	...
	while (true) {
		...
		if(size) { m_requests.dequeue(buf.data(), size); }
...
LV2_Worker_Status Lv2Worker::scheduleWork(uint32_t size, const void *data)
{
	if (m_threaded)
	{
		...
		else
		{
			...
			m_requests.enqueue(reinterpret_cast<const char*>(&size), sizeof(size));
			if(size && data) { m_requests.enqueue(reinterpret_cast<const char*>(data), size); }
			m_sem.post();
		}
	}
	else
