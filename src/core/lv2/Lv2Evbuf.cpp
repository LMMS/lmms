...
static inline uint32_t lv2_evbuf_pad_size(uint32_t size) { return (size + 7) & (~7); }
...
void lv2_evbuf_free(LV2_Evbuf* evbuf) { free(evbuf); }
...
void* lv2_evbuf_get_buffer(LV2_Evbuf* evbuf) { return &evbuf->buf; }
...
LV2_Evbuf_Iterator lv2_evbuf_next(LV2_Evbuf_Iterator iter)
{
	...
	uint32_t size;
	size = ((LV2_Atom_Event*)((char*)LV2_ATOM_CONTENTS(LV2_Atom_Sequence, &evbuf->buf.atom) + offset))->body.size;
