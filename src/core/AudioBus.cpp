...
auto AudioBus::hasInputNoise(const AudioPortsModel& apm) const -> bool
{
	auto nonQuiet = ~m_quietChannels;
	nonQuiet &= apm.out().usedTrackChannels();
	return nonQuiet.any();
}

void AudioBus::sanitize(const AudioPortsModel& apm)
{
	if (!MixHelpers::useNaNHandler()) { return; }

	const auto& channels = apm.out().usedTrackChannels();
	const auto numTrackChannels = std::min(apm.trackChannelsUpperBound(), this->channels());
...
auto AudioBus::update(const AudioPortsModel& apm) -> bool
{
	return update(apm.out().usedTrackChannels(), apm.trackChannelsUpperBound());
}
...
void AudioBus::silenceChannels(const AudioPortsModel& apm)
{
	const auto& channels = apm.out().usedTrackChannels();
