...
void ClapManager::findPlugins()
{
	...
	std::set<std::filesystem::path> searchPaths;
	...
	if (const char* clapPathTemp = getenv("CLAP_PATH"))
	{
		...
		while (pos != std::string_view::npos)
		{
			...
			if (std::filesystem::is_directory(path, ec))
				searchPaths.insert(path.make_preferred());
	...
	for (const auto& path : searchPaths)
	{
		for (const auto& entry : std::filesystem::recursive_directory_iterator(path))
		{
			...
			m_pluginPaths.insert(entry);
			if (m_debug)
				qDebug() << "-" << entry.path().c_str();
		}
	}
}
...
{
	qDebug() << "ClapManager::initPlugins";

	findPlugins();

	std::size_t pluginCount = m_pluginPaths.size();
	std::size_t pluginsLoaded = 0;
	QElapsedTimer timer;
	timer.start();

	for (auto& path : m_pluginPaths)
	{
		auto pluginLib = std::make_shared<QLibrary>();
		pluginLib->setFileName(QString::fromUtf8(path.c_str()));
		pluginLib->setLoadHints(QLibrary::ResolveAllSymbolsHint | QLibrary::DeepBindHint);
		if (!pluginLib->load())
		{
			qWarning() << pluginLib->errorString();
			continue;
		}

		auto pluginEntry = reinterpret_cast<const clap_plugin_entry_t*>(pluginLib->resolve("clap_entry"));
		if (!pluginEntry)
		{
			qWarning() << "Unable to resolve entry point 'clap_entry' in '" << path.c_str() << "'";
			pluginLib->unload();
			continue;
		}

		pluginEntry->init(path.c_str());
		auto pluginFactory = static_cast<const clap_plugin_factory_t*>(pluginEntry->get_factory(CLAP_PLUGIN_FACTORY_ID));

		auto count = pluginFactory->get_plugin_count(pluginFactory);
		if (m_debug)
			qDebug() << "plugin count:" << std::to_string(count).c_str();

		if (m_pluginIndex > count)
		{
			qWarning() << "plugin index greater than count:" << count;
			continue;
		}

		auto desc = pluginFactory->get_plugin_descriptor(pluginFactory, m_pluginIndex);
		if (!desc)
		{
		...
		auto plugin = pluginFactory->create_plugin(pluginFactory, &m_host, desc->id);
		if (!plugin)
		{
		...
		Plugin::PluginTypes type = Plugin::PluginTypes::Other;
		...
		while (features && *features)
		{
			...
			if (feature == CLAP_PLUGIN_FEATURE_INSTRUMENT)
				type = Plugin::PluginTypes::Instrument;
			else if (feature == CLAP_PLUGIN_FEATURE_AUDIO_EFFECT || feature == CLAP_PLUGIN_FEATURE_NOTE_EFFECT)
				type = Plugin::PluginTypes::Effect;
			else if (feature == CLAP_PLUGIN_FEATURE_ANALYZER)
				type = Plugin::PluginTypes::Tool;
			++features;
		}

		m_loadedPlugins.emplace_back(pluginLib, desc, type, true);
		++pluginsLoaded;
	}
