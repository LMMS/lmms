	...
	m_dpControls( this )
{
	...
	m_rms[0] = new RmsHelper( 64 * LmmsEngine::mixer()->processingSampleRate() / 44100 );
	m_rms[1] = new RmsHelper( 64 * LmmsEngine::mixer()->processingSampleRate() / 44100 );
	calcAttack();
	calcRelease();
}
...
{
	m_attCoeff = exp10( ( DNF_LOG / ( m_dpControls.m_attackModel.value() * 0.001 ) ) / LmmsEngine::mixer()->processingSampleRate() );
}

inline void dynProcEffect::calcRelease()
{
	m_relCoeff = exp10( ( -DNF_LOG / ( m_dpControls.m_releaseModel.value() * 0.001 ) ) / LmmsEngine::mixer()->processingSampleRate() );
}
...
//	qDebug( "peaks %f %f", m_currentPeak[0], m_currentPeak[1] );
	...
	if( m_needsUpdate )
	{
		m_rms[0]->setSize( 64 * LmmsEngine::mixer()->processingSampleRate() / 44100 );
		m_rms[1]->setSize( 64 * LmmsEngine::mixer()->processingSampleRate() / 44100 );
		calcAttack();
		calcRelease();
		m_needsUpdate = false;
	}
