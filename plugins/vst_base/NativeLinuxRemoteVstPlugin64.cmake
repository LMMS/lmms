IF(LMMS_BUILD_WIN64)
    RETURN()
ELSEIF(LMMS_BUILD_LINUX)
	ExternalProject_Add(NativeLinuxRemoteVstPlugin64
		"${EXTERNALPROJECT_ARGS}"
		CMAKE_ARGS
			"${EXTERNALPROJECT_CMAKE_ARGS}"
			"-DCMAKE_CXX_FLAGS=-m64 -DNATIVE_LINUX_VST"
            "-DCMAKE_C_FLAGS=-DNATIVE_LINUX_VST"
	)
	INSTALL(PROGRAMS "${CMAKE_CURRENT_BINARY_DIR}/../NativeLinuxRemoteVstPlugin64" "${CMAKE_CURRENT_BINARY_DIR}/../NativeLinuxRemoteVstPlugin64.so" DESTINATION "${PLUGIN_DIR}")
ENDIF()

