...
public:
    ...
    // Renders the current state to a QPainter (could be screen or image)
...
private:
    ...
    void drawParticles(QPainter &p, int w, int h) {
        for(auto &pt : m_particles) { 
            // Scale particle pos to current resolution
    ...
    void drawShadedCube(QPainter &p, int w, int h) {
        ...
        float angleX = m_hue*0.02f; float angleY = m_hue*0.03f; float kick = 1.0f + std::abs(g_bufferL[0]);
    ...
    , m_controls(dynamic_cast<VisualiserEffectControls*>(controls))
{
    ...
    resize(600, 480); // INCREASED HEIGHT FOR CONTROLS
    ...
    m_resCombo->addItem("Window Size");
    m_resCombo->addItem("720p HD");
    m_resCombo->addItem("1080p Full HD");
    m_resCombo->addItem("4K Ultra HD");
    
    rl->addWidget(new QLabel("Video Export:", this));
