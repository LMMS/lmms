...
extern "C"
{
	...
	Plugin::Descriptor PLUGIN_EXPORT tripleoscillator_plugin_descriptor = {STRINGIFY(PLUGIN_NAME), "TripleOscillator",
		QT_TRANSLATE_NOOP("PluginBrowser",
			"Three powerful oscillators you can modulate "
			"in several ways"),
		"Tobias Doerffel <tobydox/at/users.sf.net>", 0x0110, Plugin::Instrument, new PluginPixmapLoader("logo"), NULL,
		NULL};
}
...
OscillatorObject::OscillatorObject(Model* _parent, int _idx)
	: Model(_parent)
	, m_volumeModel(
		  DefaultVolume / NUM_OF_OSCILLATORS, MinVolume, MaxVolume, 1.0f, this, tr("Osc %1 volume").arg(_idx + 1))
	, m_panModel(DefaultPanning, PanningLeft, PanningRight, 1.0f, this, tr("Osc %1 panning").arg(_idx + 1))
	, m_coarseModel(-_idx * KeysPerOctave, -2 * KeysPerOctave, 2 * KeysPerOctave, 1.0f, this,
		  tr("Osc %1 coarse detuning").arg(_idx + 1))
	, m_fineLeftModel(0.0f, -100.0f, 100.0f, 1.0f, this, tr("Osc %1 fine detuning left").arg(_idx + 1))
	, m_fineRightModel(0.0f, -100.0f, 100.0f, 1.0f, this, tr("Osc %1 fine detuning right").arg(_idx + 1))
	, m_phaseOffsetModel(0.0f, 0.0f, 360.0f, 1.0f, this, tr("Osc %1 phase-offset").arg(_idx + 1))
	, m_stereoPhaseDetuningModel(0.0f, 0.0f, 360.0f, 1.0f, this, tr("Osc %1 stereo phase-detuning").arg(_idx + 1))
	, m_waveShapeModel(
		  Oscillator::SineWave, 0, Oscillator::NumWaveShapes - 1, this, tr("Osc %1 wave shape").arg(_idx + 1))
	, m_modulationAlgoModel(
		  Oscillator::SignalMix, 0, Oscillator::NumModulationAlgos - 1, this, tr("Modulation type %1").arg(_idx + 1))
	,
	...
	, m_phaseOffsetRight(0.0f)
{
	...
	connect(&m_panModel, SIGNAL(dataChanged()), this, SLOT(updateVolume()), Qt::DirectConnection);
	updateVolume();

	connect(&m_coarseModel, SIGNAL(dataChanged()), this, SLOT(updateDetuningLeft()), Qt::DirectConnection);
	connect(&m_coarseModel, SIGNAL(dataChanged()), this, SLOT(updateDetuningRight()), Qt::DirectConnection);
	connect(&m_fineLeftModel, SIGNAL(dataChanged()), this, SLOT(updateDetuningLeft()), Qt::DirectConnection);
	connect(&m_fineRightModel, SIGNAL(dataChanged()), this, SLOT(updateDetuningRight()), Qt::DirectConnection);
	updateDetuningLeft();
	updateDetuningRight();

	connect(&m_phaseOffsetModel, SIGNAL(dataChanged()), this, SLOT(updatePhaseOffsetLeft()), Qt::DirectConnection);
	connect(&m_phaseOffsetModel, SIGNAL(dataChanged()), this, SLOT(updatePhaseOffsetRight()), Qt::DirectConnection);
	connect(
		&m_stereoPhaseDetuningModel, SIGNAL(dataChanged()), this, SLOT(updatePhaseOffsetLeft()), Qt::DirectConnection);
	updatePhaseOffsetLeft();
	updatePhaseOffsetRight();
}

OscillatorObject::~OscillatorObject() { sharedObject::unref(m_sampleBuffer); }
...
{
	m_phaseOffsetLeft = (m_phaseOffsetModel.value() + m_stereoPhaseDetuningModel.value()) / 360.0f;
}

void OscillatorObject::updatePhaseOffsetRight() { m_phaseOffsetRight = m_phaseOffsetModel.value() / 360.0f; }
	...
	: Instrument(_instrument_track, &tripleoscillator_plugin_descriptor)
{
	...
	connect(Engine::mixer(), SIGNAL(sampleRateChanged()), this, SLOT(updateAllDetuning()));
}
...
void TripleOscillator::saveSettings(QDomDocument& _doc, QDomElement& _this)
{
	for (int i = 0; i < NUM_OF_OSCILLATORS; ++i)
	{
		...
		m_osc[i]->m_phaseOffsetModel.saveSettings(_doc, _this, "phoffset" + is);
		m_osc[i]->m_stereoPhaseDetuningModel.saveSettings(_doc, _this, "stphdetun" + is);
		m_osc[i]->m_waveShapeModel.saveSettings(_doc, _this, "wavetype" + is);
		m_osc[i]->m_modulationAlgoModel.saveSettings(_doc, _this, "modalgo" + QString::number(i + 1));
...
void TripleOscillator::loadSettings(const QDomElement& _this)
{
	for (int i = 0; i < NUM_OF_OSCILLATORS; ++i)
	{
		...
		m_osc[i]->m_stereoPhaseDetuningModel.loadSettings(_this, "stphdetun" + is);
		m_osc[i]->m_waveShapeModel.loadSettings(_this, "wavetype" + is);
		m_osc[i]->m_modulationAlgoModel.loadSettings(_this, "modalgo" + QString::number(i + 1));
		m_osc[i]->m_sampleBuffer->setAudioFile(_this.attribute("userwavefile" + is));
	}
}

QString TripleOscillator::nodeName() const { return (tripleoscillator_plugin_descriptor.name); }
...
PluginView* TripleOscillator::instantiateView(QWidget* _parent) { return new TripleOscillatorView(this, _parent); }
...
TripleOscillatorView::TripleOscillatorView(Instrument* _instrument, QWidget* _parent)
	: InstrumentViewFixedSize(_instrument, _parent)
{
	...
	pm_osc1_btn->setInactiveGraphic(PLUGIN_NAME::getIconPixmap("pm_inactive"));
	...
	am_osc1_btn->setInactiveGraphic(PLUGIN_NAME::getIconPixmap("am_inactive"));
	...
	mix_osc1_btn->setInactiveGraphic(PLUGIN_NAME::getIconPixmap("mix_inactive"));
	...
	sync_osc1_btn->setActiveGraphic(PLUGIN_NAME::getIconPixmap("sync_active"));
	sync_osc1_btn->setInactiveGraphic(PLUGIN_NAME::getIconPixmap("sync_inactive"));
	ToolTip::add(sync_osc1_btn,
		tr("Synchronize oscillator 1 with "
		   "oscillator 2"));
	...
	fm_osc1_btn->setInactiveGraphic(PLUGIN_NAME::getIconPixmap("fm_inactive"));
	...
	pm_osc2_btn->setInactiveGraphic(PLUGIN_NAME::getIconPixmap("pm_inactive"));
	...
	am_osc2_btn->setInactiveGraphic(PLUGIN_NAME::getIconPixmap("am_inactive"));
	...
	mix_osc2_btn->setInactiveGraphic(PLUGIN_NAME::getIconPixmap("mix_inactive"));
	...
	sync_osc2_btn->setActiveGraphic(PLUGIN_NAME::getIconPixmap("sync_active"));
	sync_osc2_btn->setInactiveGraphic(PLUGIN_NAME::getIconPixmap("sync_inactive"));
	...
	fm_osc2_btn->setInactiveGraphic(PLUGIN_NAME::getIconPixmap("fm_inactive"));
	...
	for (int i = 0; i < NUM_OF_OSCILLATORS; ++i)
	{
		...
		ck->move(82, knob_y);
		ck->setHintText(tr("Osc %1 coarse detuning:").arg(i + 1), " " + tr("semitones"));

		// setup knob for left fine-detuning
		Knob* flk = new TripleOscKnob(this);
		flk->move(111, knob_y);
		flk->setHintText(tr("Osc %1 fine detuning left:").arg(i + 1), " " + tr("cents"));

		// setup knob for right fine-detuning
		Knob* frk = new TripleOscKnob(this);
		frk->move(140, knob_y);
		frk->setHintText(tr("Osc %1 fine detuning right:").arg(i + 1), " " + tr("cents"));

		// setup phase-offset-knob
		Knob* pok = new TripleOscKnob(this);
		pok->move(188, knob_y);
		pok->setHintText(tr("Osc %1 phase-offset:").arg(i + 1), " " + tr("degrees"));

		// setup stereo-phase-detuning-knob
		Knob* spdk = new TripleOscKnob(this);
		spdk->move(217, knob_y);
		spdk->setHintText(tr("Osc %1 stereo phase-detuning:").arg(i + 1), " " + tr("degrees"));

		int btn_y = 96 + i * osc_h;

		PixmapButton* sin_wave_btn = new PixmapButton(this, NULL);
		sin_wave_btn->move(128, btn_y);
		sin_wave_btn->setActiveGraphic(PLUGIN_NAME::getIconPixmap("sin_shape_active"));
		sin_wave_btn->setInactiveGraphic(PLUGIN_NAME::getIconPixmap("sin_shape_inactive"));
		...
		triangle_wave_btn->setActiveGraphic(PLUGIN_NAME::getIconPixmap("triangle_shape_active"));
		triangle_wave_btn->setInactiveGraphic(PLUGIN_NAME::getIconPixmap("triangle_shape_inactive"));
		...
		saw_wave_btn->setActiveGraphic(PLUGIN_NAME::getIconPixmap("saw_shape_active"));
		saw_wave_btn->setInactiveGraphic(PLUGIN_NAME::getIconPixmap("saw_shape_inactive"));
		ToolTip::add(saw_wave_btn, tr("Saw wave"));

		PixmapButton* sqr_wave_btn = new PixmapButton(this, NULL);
		sqr_wave_btn->move(173, btn_y);
		sqr_wave_btn->setActiveGraphic(PLUGIN_NAME::getIconPixmap("square_shape_active"));
		sqr_wave_btn->setInactiveGraphic(PLUGIN_NAME::getIconPixmap("square_shape_inactive"));
		...
		moog_saw_wave_btn->setActiveGraphic(PLUGIN_NAME::getIconPixmap("moog_saw_shape_active"));
		moog_saw_wave_btn->setInactiveGraphic(PLUGIN_NAME::getIconPixmap("moog_saw_shape_inactive"));
		ToolTip::add(moog_saw_wave_btn, tr("Moog-like saw wave"));

		PixmapButton* exp_wave_btn = new PixmapButton(this, NULL);
		exp_wave_btn->move(203, btn_y);
		exp_wave_btn->setActiveGraphic(PLUGIN_NAME::getIconPixmap("exp_shape_active"));
		exp_wave_btn->setInactiveGraphic(PLUGIN_NAME::getIconPixmap("exp_shape_inactive"));
		...
		white_noise_btn->setActiveGraphic(PLUGIN_NAME::getIconPixmap("white_noise_shape_active"));
		white_noise_btn->setInactiveGraphic(PLUGIN_NAME::getIconPixmap("white_noise_shape_inactive"));
		...
		uwb->setInactiveGraphic(PLUGIN_NAME::getIconPixmap("usr_shape_inactive"));
		...
		m_oscKnobs[i] = OscillatorKnobs(vk, pk, ck, flk, frk, pok, spdk, uwb, wsbg);
	}
}
...
void TripleOscillatorView::modelChanged()
{
	...
	for (int i = 0; i < NUM_OF_OSCILLATORS; ++i)
	{
		...
		m_oscKnobs[i].m_fineRightKnob->setModel(&t->m_osc[i]->m_fineRightModel);
		m_oscKnobs[i].m_phaseOffsetKnob->setModel(&t->m_osc[i]->m_phaseOffsetModel);
		m_oscKnobs[i].m_stereoPhaseDetuningKnob->setModel(&t->m_osc[i]->m_stereoPhaseDetuningModel);
		m_oscKnobs[i].m_waveShapeBtnGrp->setModel(&t->m_osc[i]->m_waveShapeModel);
		connect(m_oscKnobs[i].m_userWaveButton, SIGNAL(doubleClicked()), t->m_osc[i], SLOT(oscUserDefWaveDblClick()));
	}
}
