...
void OscillatorObject::applyPhaseRandomness()
{
	...
	Instrument( _instrument_track, &tripleoscillator_plugin_descriptor )
{
	...
	connect( instrumentTrack()->pitchRangeModel(),
			SIGNAL( dataChanged() ),
...
void TripleOscillator::updateAllPitchRange()
{
	const int range = instrumentTrack()->pitchRangeModel()->value();
	for( int i = 0; i < NUM_OF_OSCILLATORS; ++i )
	{
