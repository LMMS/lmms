	...
	, m_nextPlayBackwards(false)
{
	...
	connect(&m_endPointModel, SIGNAL(dataChanged()), this, SLOT(endPointChanged()), Qt::DirectConnection);
	connect(&m_loopPointModel, SIGNAL(dataChanged()), this, SLOT(loopPointChanged()), Qt::DirectConnection);
	connect(&m_stutterModel, SIGNAL(dataChanged()), this, SLOT(stutterModelChanged()), Qt::DirectConnection);
...
void audioFileProcessor::playNote(NotePlayHandle* _n, sampleFrame* _working_buffer)
{
	...
	if (!_n->isFinished())
	{
		if (m_sampleBuffer.play(_working_buffer + offset, (handleState*)_n->m_pluginData, frames, _n->frequency(),
...
void audioFileProcessor::loadSettings(const QDomElement& _this)
{
	...
	if (_this.hasAttribute("interp")) { m_interpolationModel.loadSettings(_this, "interp"); }
	else
	{
...
void audioFileProcessor::loadFile(const QString& _file) { setAudioFile(_file); }
...
void audioFileProcessor::startPointChanged(void)
{
	...
	if (m_loopPointModel.value() < m_startPointModel.value()) { m_loopPointModel.setValue(m_startPointModel.value()); }
...
void audioFileProcessor::loopPointChanged(void)
{
	...
	if (m_loopPointModel.value() < m_startPointModel.value()) { m_startPointModel.setValue(m_loopPointModel.value()); }
	...
	: InstrumentViewFixedSize(_instrument, _parent)
{
	...
	connect(castModel<audioFileProcessor>(), SIGNAL(isPlaying(f_cnt_t)), m_waveView, SLOT(isPlaying(f_cnt_t)));
...
void AudioFileProcessorView::dragEnterEvent(QDragEnterEvent* _dee)
{
	...
	if (_dee->mimeData()->hasFormat(mimeType(MimeType::StringPair)))
	{
		...
		if (txt.section(':', 0, 0) == QString("tco_%1").arg(Track::SampleTrack)) { _dee->acceptProposedAction(); }
...
void AudioFileProcessorView::newWaveView()
{
	...
	m_waveView->setKnobs(dynamic_cast<AudioFileProcessorWaveView::knob*>(m_startKnob),
...
void AudioFileProcessorWaveView::enterEvent(QEvent* _e) { updateCursor(); }
...
void AudioFileProcessorWaveView::mousePressEvent(QMouseEvent* _me)
{
	...
	draggingType dt = sample_loop;
	int md = loop_dist;
	if (start_dist < loop_dist)
	{
		dt = sample_start;
		md = start_dist;
	}
	else if (end_dist < loop_dist)
	{
		dt = sample_end;
		md = end_dist;
	}

	if (md < 4) { m_draggingType = dt; }
	else
	{
		m_draggingType = wave;
		updateCursor(_me);
...
void AudioFileProcessorWaveView::paintEvent(QPaintEvent* _pe)
{
	...
	m_startFrameX = graph_rect.x() + (m_sampleBuffer.startFrame() - m_from) * double(graph_rect.width()) / frames;
	m_endFrameX = graph_rect.x() + (m_sampleBuffer.endFrame() - m_from) * double(graph_rect.width()) / frames;
	m_loopFrameX = graph_rect.x() + (m_sampleBuffer.loopStartFrame() - m_from) * double(graph_rect.width()) / frames;
	const int played_width_px = (m_framesPlayed - m_from) * double(graph_rect.width()) / frames;

	// loop point line
	p.setPen(QColor(0x7F, 0xFF, 0xFF)); // TODO: put into a qproperty
	p.drawLine(m_loopFrameX, graph_rect.y(), m_loopFrameX, graph_rect.height() + graph_rect.y());

	// start/end lines
	p.setPen(QColor(0xFF, 0xFF, 0xFF)); // TODO: put into a qproperty
	p.drawLine(m_startFrameX, graph_rect.y(), m_startFrameX, graph_rect.height() + graph_rect.y());
	p.drawLine(m_endFrameX, graph_rect.y(), m_endFrameX, graph_rect.height() + graph_rect.y());

	if (m_endFrameX - m_startFrameX > 2)
	{
		p.fillRect(m_startFrameX + 1, graph_rect.y(), m_endFrameX - m_startFrameX - 1,
			graph_rect.height() + graph_rect.y(), QColor(95, 175, 255, 50) // TODO: put into a qproperty
		);
		if (m_endFrameX - m_loopFrameX > 2)
			p.fillRect(m_loopFrameX + 1, graph_rect.y(), m_endFrameX - m_loopFrameX - 1,
				graph_rect.height() + graph_rect.y(), QColor(95, 205, 255, 65) // TODO: put into a qproperty
			);
		...
		if (m_framesPlayed && m_animation)
		{
			...
			g.setColorAt(0.8, c);
			g.setColorAt(1, c);
			p.fillRect(m_startFrameX + 1, graph_rect.y(), played_width_px - (m_startFrameX + 1),
				graph_rect.height() + graph_rect.y(), g);
			p.setPen(QColor(255, 255, 255)); // TODO: put into a qproperty
			p.drawLine(played_width_px, graph_rect.y(), played_width_px, graph_rect.height() + graph_rect.y());
			m_framesPlayed = 0;
		}
	}
	...
	if (length > 20000) { length_text = QString::number(length / 1000) + "s"; }
	else if (length > 2000)
	{
...
void AudioFileProcessorWaveView::updateGraph()
{
	...
	if (m_from > m_sampleBuffer.startFrame()) { m_from = m_sampleBuffer.startFrame(); }

	if (m_to < m_sampleBuffer.endFrame()) { m_to = m_sampleBuffer.endFrame(); }

	if (m_sampleBuffer.reversed() != m_reversed) { reverse(); }
...
void AudioFileProcessorWaveView::zoom(const bool _out)
{
	...
	const double comp_ratio = double(qMin(d_from, d_to)) / qMax(1, qMax(d_from, d_to));
	...
	if ((_out && d_from < d_to) || (!_out && d_to < d_from))
	{
		...
		new_to = qBound(end, m_to + f_cnt_t(step_to * (new_from == m_from ? 1 : comp_ratio)), frames);
	}
	else
	{
		new_to = qBound(end, m_to + step_to, frames);
		new_from = qBound(0, m_from + f_cnt_t(step_from * (new_to == m_to ? 1 : comp_ratio)), start);
	}
