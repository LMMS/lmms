/*
 * Arpeggiator.cpp - A native arpeggiatorer
 *
...
Plugin::Descriptor PLUGIN_EXPORT arpeggiator_plugin_descriptor =
{
	STRINGIFY( PLUGIN_NAME ),
	"Arpeggiator",
	QT_TRANSLATE_NOOP( "pluginBrowser", "An oversampling arpeggiatorer" ),
...
ArpeggiatorEffect::ArpeggiatorEffect( Model * parent, const Descriptor::SubPluginFeatures::Key * key ) :
	Effect( &arpeggiator_plugin_descriptor, parent, key ),
	m_controls( this ),
...
void ArpeggiatorEffect::sampleRateChanged()
{
	m_sampleRate = Engine::mixer()->processingSampleRate();
...
inline float ArpeggiatorEffect::depthCrush( float in )
{
	return roundf( in * (float) m_levels ) * m_levelsRatio;
}

inline float ArpeggiatorEffect::noise( float amt )
{
	return fastRandf( amt * 2.0f ) - amt;
}

bool ArpeggiatorEffect::processAudioBuffer( sampleFrame* buf, const fpp_t frames )
{
...
{
	return new ArpeggiatorEffect( parent, static_cast<const Plugin::Descriptor::SubPluginFeatures::Key *>( data ) );
}
