...
vibratingString::vibratingString(float _pitch, float _pick, float _pickup, float* _impulse, int _len,
	sample_rate_t _sample_rate, int _oversample, float _randomize, float _string_loss, float _detune, bool _state)
	...
	, m_state(0.1f)
{
	...
	string_length = static_cast<int>(m_oversample * _sample_rate / _pitch) + 1;
	...
	vibratingString::setDelayLine(m_toBridge, pick, m_impulse, _len, 0.5f, _state);
	vibratingString::setDelayLine(m_fromBridge, pick, m_impulse, _len, 0.5f, _state);

	m_choice = static_cast<int>(m_oversample * static_cast<float>(rand()) / RAND_MAX);
...
void vibratingString::resample(float* _src, f_cnt_t _src_frames, f_cnt_t _dst_frames)
{
	for (f_cnt_t frame = 0; frame < _dst_frames; ++frame)
	{
		...
		m_impulse[frame] = cubicInterpolate(
			_src[src_frame - 1], _src[src_frame + 0], _src[src_frame + 1], _src[src_frame + 2], frac_pos);
	}
}
