...
#define decimal(p) (((p)[0] - '0') * 10 + ((p)[1] - '0'))
	...
	if (keysig != -99 && keysig_mode)
	{ // write when both are defined
...
void Alg_smf_write::write_track(int i)
{
	...
	{ // track 0 may have tempo and timesig info
		...
		{
			pending = push(pending, new event_queue('s', TICK_TIME(seq->time_sig[0].beat, 0), 0, NULL));
		}
	}
	while (pending)
	{
		...
		if (current->type == 'n')
		{
			...
			{
				write_note(n, true);
				pending = push(pending, new event_queue('o', TICK_TIME(n->time + n->dur, -1), current->index, NULL));
			}
			else if (n->is_update())
			{
...
//  i is index of tempo map
{
	...
	{
		tempo = 1000000 * ((b[i + 1].time - b[i].time) / (b[i + 1].beat - b[i].beat));
...
void Alg_smf_write::write_time_signature(int i)
{
	...
	int den_byte = 0;
	while (den > 1)
	{ // compute the log2 of denominator
		den_byte++;
		den >>= 1;
	}
