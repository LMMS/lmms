...
void Alg_midifile_reader::Mf_keysig(int key, int mode)
{
	...
	mode_parm.a = (mode == 0 ? symbol_table.insert_string("major") : symbol_table.insert_string("minor"));
...
Alg_error alg_smf_read(istream& file, Alg_seq_ptr new_seq)
{
	...
	bool err = ar.parse();
	ar.seq->set_real_dur(ar.seq->get_time_map()->beat_to_time(ar.seq->get_beat_dur()));
