	...
	m_active ( true )
{
	...
	{
		m_fftWindow[i] = (a0 - a1 * cos(2 * numbers::pi_v<float> * i / ((float)FFT_BUFFER_SIZE - 1.0))
			+ a2 * cos(4 * numbers::pi_v<float> * i / ((float)FFT_BUFFER_SIZE - 1.0))
			- a3 * cos(6 * numbers::pi_v<float> * i / ((float)FFT_BUFFER_SIZE - 1.0)));
	}
	clear();
}
