...
void lb302Filter::recalc()
{
	...
	vcf_e0*=M_PI/LmmsEngine::mixer()->processingSampleRate();
	vcf_e1*=M_PI/LmmsEngine::mixer()->processingSampleRate();
	vcf_e1 -= vcf_e0;
...
void lb302Filter3Pole::envRecalc()
{
	...
	kfco = 50.f + (k)*((2300.f-1600.f*(fs->envmod))+(w) *
	                   (700.f+1500.f*(k)+(1500.f+(k)*(LmmsEngine::mixer()->processingSampleRate()/2.f-6000.f)) *
	                   (fs->envmod)) );
	...
	// kfcn = fs->cutoff;
	kfcn = 2.0 * kfco / LmmsEngine::mixer()->processingSampleRate();
#else
	kfcn = w;
#endif
...
void lb302Synth::filterChanged()
{
	...
	d *= LmmsEngine::mixer()->processingSampleRate();                                // d *= smpl rate
...
inline float GET_INC(float freq) {
	return freq/LmmsEngine::mixer()->processingSampleRate();  // TODO: Use actual sampling rate.
}

int lb302Synth::process(sampleFrame *outbuf, const int size)
{
	const float sampleRatio = 44100.f / LmmsEngine::mixer()->processingSampleRate();
	float w;
	float samp;
	...
	//			vca_a = 0;
		...
		*/
		...
		if(vca_mode==0) {
			...
			if(sample_cnt>=0.5*LmmsEngine::mixer()->processingSampleRate())
				vca_mode = 2;
		}
		else if(vca_mode == 1) {
			vca_a *= vca_decay;
