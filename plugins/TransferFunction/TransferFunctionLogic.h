...
inline Complex calculatePoleZeroResponse(float freq, float fs, 
                                         ...
                                         const std::vector<Complex>& zeros)
{
    ...
    if (std::abs(denominator) < 1e-9f) return Complex(1.0f, 0.0f); 
    return numerator / denominator;
}
// =============================================================
// FORMULA STRING HELPER
// =============================================================
...
inline std::string getPresetFormulaString(int presetIdx) {
    switch(presetIdx) {
        ...
        case 19: return "Spectral Reverb: exp(j * Random(freq))"; 
        case 20: return "Pole/Zero Editor: Design Z-Plane Filter";
...
inline Complex calculateTransferFunction(int presetIdx, float freq, 
                                         ...
                                         const std::vector<Complex>* poles = nullptr,
                                         const std::vector<Complex>* zeros = nullptr,
                                         float fs = 44100.0f)
{
    Complex H(1.0f, 0.0f);
    float fc, bw, delay; 
    ...
    else if (presetIdx == 20)
    {
        if (poles && zeros) {
            H = calculatePoleZeroResponse(freq, fs, *poles, *zeros);
        } else {
            H = Complex(1.0f, 0.0f);
        }
    }
    ...
    {
        switch(presetIdx) {
            case 2: H = 1.0f / std::sqrt(1.0f + std::pow(freq / 800.0f, 8.0f)); break;
            case 3: if (freq < 1.0f) freq = 1.0f; H = 1.0f / std::sqrt(1.0f + std::pow(500.0f / freq, 8.0f)); break;
            case 4: H = 1.0f + 1.0f / (1.0f + std::pow(freq / 500.0f, 2.0f)); break;
            case 5: H = 1.0f + 1.0f / (1.0f + std::pow(2000.0f / (freq + 1.0f), 2.0f)); break;
            case 6: if (freq > 300.0f && freq < 3000.0f) { H = Complex(1.0f, 0.0f); } else { H = Complex(0.0f, 0.0f); } break;
            case 7: fc = 600.0f; bw = 40.0f; H = 1.0f - std::exp(-std::pow(freq - fc, 2.0f) / (2.0f * bw * bw)); break;
            case 8: fc = 900.0f; H = 1.0f + 8.0f * std::exp(-std::pow(20.0f * (freq - fc) / fc, 2.0f)); break;
            case 9: delay = 0.002f; H = 1.0f + std::exp(Complex(0, -1) * 2.0f * PI_LOGIC * freq * delay); break;
            case 10: delay = 0.003f; H = 1.0f - std::exp(Complex(0, -1) * 2.0f * PI_LOGIC * freq * delay); break;
            case 11: fc = 1200.0f; H = (1.0f - Complex(0,1) * (freq/fc)) / (1.0f + Complex(0,1) * (freq/fc)); break;
            case 12: H = (std::fmod(freq, 500.0f) < 250.0f) ? 1.0f : -1.0f; break;
            case 13: H = std::exp(-0.0004f * freq); break;
            case 14: if (freq > 500.0f) { H = std::exp(-0.0008f * (freq - 500.0f)) * std::sin(freq * 0.005f); } else { H = 0.0f; } break;
            case 15: H = std::exp(-std::pow(freq-800.0f, 2.0f)/(2.0f*6400.0f)) + std::exp(-std::pow(freq-1500.0f, 2.0f)/(2.0f*22500.0f)) + std::exp(-std::pow(freq-2500.0f, 2.0f)/(2.0f*40000.0f)); break;
            case 16: H = 1.0f + 0.8f * std::sin(PI_LOGIC * freq / 200.0f); break;
            case 17: H = std::exp(Complex(0, 1) * 0.0005f * freq); break;
       
            case 19: 
            {
                float hash = std::abs(std::sin(freq * 12.9898f + 0.123f)) * 43758.5453f;
                float randomPhase = (hash - std::floor(hash)) * 2.0f * PI_LOGIC;
                H = std::exp(Complex(0.0f, 1.0f) * randomPhase);
                break;
            }
            default: 
                H = Complex(1.0f, 0.0f); 
