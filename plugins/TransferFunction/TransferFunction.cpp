...
// --- MATH PARSER (From your requested file) ---
class SimpleParser {
    std::string str;
    size_t pos;
    float currentFreq;
...
Effect::ProcessStatus TransferFunctionEffect::processImpl(SampleFrame* buf, const fpp_t frames) {
    ...
    // Declare Parser (static to avoid reallocation, but be careful with threads)
    ...
    for (int i = 0; i <= N/2; ++i) {
        ...
        Complex H(1.0f, 0.0f);
        float fc, bw, delay; // Temp vars for switch
        ...
        if (presetIdx == 1) {
            ...
            switch(presetIdx) {
                ...
                case 4: // Low Shelf
                    H = 1.0f + (1.0f) / (1.0f + std::pow(freq / 200.0f, 2.0f)); break;
                case 5: // High Shelf
