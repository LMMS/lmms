...
void sf2Instrument::updateSampleRate()
{
	...
	fluid_settings_setnum( m_settings, (char *) "synth.sample-rate", LmmsEngine::mixer()->processingSampleRate() );
	...
	m_synthMutex.lock();
	if( LmmsEngine::mixer()->currentQualitySettings().interpolation >=
	...
	m_synthMutex.unlock();
	if( m_internalSampleRate < LmmsEngine::mixer()->processingSampleRate() )
	{
		m_synthMutex.lock();
		if( m_srcState != NULL )
		{
		...
		int error;
		m_srcState = src_new( LmmsEngine::mixer()->currentQualitySettings().libsrcInterpolation(), DEFAULT_CHANNELS, &error );
...
void sf2Instrument::renderFrames( f_cnt_t frames, sampleFrame * buf )
{
	...
	if( m_internalSampleRate < LmmsEngine::mixer()->processingSampleRate() &&
							m_srcState != NULL )
	{
		const fpp_t f = frames * m_internalSampleRate / LmmsEngine::mixer()->processingSampleRate();
#ifdef __GNUC__
		sampleFrame tmp[f];
#else
