...
bool StereoControlEffect::processAudioBuffer( sampleFrame* buf, const fpp_t frames )
{
	...
	for( fpp_t f = 0; f < frames; ++f )
	{
		...
		const float panSpectral = panSpectralBuf ? panSpectralBuf->value( f ) : m_stereocontrolControls.m_panSpectralModel.value();
		...
		for (int i = 0; i < 2; ++i)
		{
			...
			float hp;
			float hp2;
			float g = tan(F_PI * stereoizerHP / m_sampleRate);
			g /= (1.f + g);
			multimodeFilter(allpassValue, g, lp, hp, m_stereoizerFilter[0][i]);
			float g2 = tan(F_PI * stereoizerLP / m_sampleRate);
			g2 /= (1.f + g2);
			multimodeFilter(hp, g2, lp, hp2, m_stereoizerFilter[1][i]);
		...
		if (monoBass)
		{
			float g = tan(F_PI * monoBassFreq / m_sampleRate);
			g /= (1.f + g);
			...
			for (int i = 0 ; i < 2; ++i)
			{
				multimodeFilter(s[i], g, lp[i], hp[i], m_monoBassFilter[i]);
			}
		...
		switch (panMode)
		{
			...
			case 2:// Haas
			{
				...
				if (pan >= 0)
				{
					const float lGain = ((pan > 0 ? 1.f - pan : 1.f) + minGain) / (1.f + minGain);
					s[0] = haasResult * lGain;
				}
				else
				{
					const float rGain = ((pan < 0 ? 1.f + pan : 1.f) + minGain) / (1.f + minGain);
