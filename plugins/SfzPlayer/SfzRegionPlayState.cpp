...
bool SfzRegionPlayState::play(SampleFrame* buffer, const f_cnt_t frames)
{
	...
	for (f_cnt_t f = 0; f < frames; ++f)
	{
		...
		{
			const double frameIncrement = m_baseFreqRatio * notePlayHandleFreqRatio * pitchmodFreqRatio; // Apply the pitch modulation by speeding up/slowing down the playback
			m_sampleFrame = m_sampleObject != nullptr
				? std::min(static_cast<double>(m_sampleObject->size()), m_sampleFrame + frameIncrement)
